    ...
    it('TC-301-3 Meal successfully added', (done) => {
      chai.request(server).post('/api/meal').send({
        ...
        .end((err, res) => {
          ...
          data[0].should.has.property('isVega').to.be.equal(1);
          data[0].should.has.property('isToTakeHome').to.be.equal(1);
          data[0].should.has.property('dateTime').to.be.equal('2023-05-22 17:35:00');
          data[0].should.has.property('maxAmountOfParticipants').to.be.equal(3);
          data[0].should.has.property('price').to.be.equal(9);
          data[0].should.has.property('imageUrl').to.be.equal('url');
          data[0].should.has.property('cookId').to.be.equal(1);
          data[0].should.has.property('name').to.be.equal('Spaghetti');
          data[0].should.has.property('description').to.be.equal('Beste Spaghetti');
          data[0].should.has.property('allergenes').to.be.equal('gluten');
          done();
        });
    });
  });
    ...
    it('TC-301-5 Meal successfully edited', (done) => {
      chai.request(server).put('/api/meal/1').send({
        ...
        .end((err, res) => {
          ...
          data[0].should.has.property('imageUrl').to.be.equal('url');
          data[0].should.has.property('cookId').to.be.equal(1);
          data[0].should.has.property('name').to.be.equal('Spaghetti (edit)');
          data[0].should.has.property('description').to.be.equal('Beste Spaghetti');
          data[0].should.has.property('allergenes').to.be.equal('gluten');
          done();
        });
